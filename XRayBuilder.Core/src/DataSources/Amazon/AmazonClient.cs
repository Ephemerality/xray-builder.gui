...
namespace XRayBuilder.Core.DataSources.Amazon
{
    ...
    public class AmazonClient : IAmazonClient
    {
        ...
        public async Task<AuthorSearchResults> SearchAuthor(string author, string bookAsin, string TLD, CancellationToken cancellationToken, bool enableLog)
        {
            ...
            if (node == null || !node.OuterHtml.Contains("/e/B"))
            {
                ...
                {
                    if(enableLog) _logger.Log($"An error occurred finding author's page on Amazon.{TLD}." +
            ...
            {
                if(enableLog) _logger.Log("Unable to parse author's page URL properly. Try again later or report this URL on the MobileRead thread: " + amazonAuthorSearchUrl);
                return null;
            }
