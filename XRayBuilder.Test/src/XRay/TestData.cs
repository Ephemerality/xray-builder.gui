...
namespace XRayBuilder.Test.XRay
{
    ...
    {
        public static XRayBuilderGUI.XRay.XRay CreateXRayFromXML(string path, string db, string guid, string asin, Goodreads goodreads, ILogger logger, IAliasesService aliasesService, ChaptersService chaptersService)
            => new XRayBuilderGUI.XRay.XRay(path, db, guid, asin, goodreads, logger, aliasesService, chaptersService, true, 0, "") { Unattended = true };
