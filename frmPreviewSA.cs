...
namespace XRayBuilderGUI
{
    public partial class frmPreviewSA : Form
    {
        ...
        public bool populateStartActions(string inputFile)
        {
            ...
            //
            Match previousBookInTheSeries = Regex.Match(input, @"""previousBookInTheSeries"":{(.*)},""authorSubscriptions""");
            if (!previousBookInTheSeries.Success)
                previousBookInTheSeries = Regex.Match(input, @"""previousBookInTheSeries"":{(.*)}}");
            if (previousBookInTheSeries.Success)
            {
